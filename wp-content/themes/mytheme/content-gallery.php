...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header">
		<hgroup>
			...
			<h3 class="entry-format"><?php _e( 'Gallery', 'mytheme' ); ?></h3>
		</hgroup>
	...
	<?php if ( is_search() ) : // Only display Excerpts for search pages ?>
		...
		<div class="entry-content">
			...
			<?php else :
				$images = mytheme_get_gallery_images();
				if ( $images ) :
			...
			?>
				...
				<p><em><?php printf( _n( 'This gallery contains <a %1$s>%2$s photo</a>.', 'This gallery contains <a %1$s>%2$s photos</a>.', $total_images, 'mytheme' ),
						'href="' . esc_url( get_permalink() ) . '" title="' . esc_attr( sprintf( __( 'Permalink to %s', 'mytheme' ), the_title_attribute( 'echo=0' ) ) ) . '" rel="bookmark"',
						number_format_i18n( $total_images )
					); ?></em></p>
			<?php endif; // end mytheme_get_gallery_images() check ?>
			<?php the_excerpt(); ?>
		<?php endif; ?>
